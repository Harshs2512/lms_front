  ...
  return (
      <div className="bg-white justify-center items-center py-2">
        ...
        <div className="md:px-10 md:grid md:grid-cols-2 lg:grid-cols-3 space-y-4 md:space-y-0 py-8 md:gap-4">
          {courses?.map((p) => (
            <div key={p._id}>
              <div className=" w-80 h-96 mb-5 bg-white px-6 pt-6 pb-2 rounded-xl shadow-lg transform hover:scale-105 transition duration-500">
                ...
                <h1 className="mt-4 text-gray-800 text-left font-bold cursor-pointer p-1 pb-2">{p.title}</h1>
                <h1 className="mt-4 text-gray-800 text-left cursor-pointer p-1 flex"><span><UilUserCircle/></span>Cybrom</h1>
                <hr />
                <div className="my-1">
                  <div className="flex space-x-2 text-[12px] items-center">
                    <p className="p-1 flex text-sm">
                    <UilPlayCircle/> <span>Lessons</span>
                    </p>
                    <p>{p.catName}</p>
                  </div>
